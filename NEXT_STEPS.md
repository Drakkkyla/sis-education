# Следующие шаги после сборки проекта

## ✅ Что уже сделано:
- ✅ Проект собран (backend/dist/ и frontend/dist/ созданы)
- ✅ Все файлы готовы к загрузке

## 📤 Шаг 1: Загрузка файлов на хостинг через SFTP

### Что нужно загрузить:

1. **backend/** (всю папку)
   - `backend/dist/` - скомпилированный код
   - `backend/package.json` и `package-lock.json`
   - `backend/uploads/` (папка должна существовать)

2. **frontend/dist/** (только папку dist)
   - Все файлы из `frontend/dist/`

3. **photo/** (если есть логотипы курсов)

### Как загрузить:

**Вариант 1: FileZilla или WinSCP**
- Подключитесь к серверу через SFTP
- Перетащите папки на сервер в корневую директорию (обычно `/home/container/`)

**Вариант 2: Командная строка (scp)**
```bash
scp -r backend username@your-server.com:/home/container/
scp -r frontend/dist username@your-server.com:/home/container/frontend/
scp -r photo username@your-server.com:/home/container/
```

## ⚙️ Шаг 2: Настройка на сервере

### 1. Создайте файл `backend/.env`:

```env
PORT=5000
MONGODB_URI=mongodb+srv://username:password@cluster.mongodb.net/kvantorium
JWT_SECRET=ваш-случайный-секретный-ключ-минимум-32-символа
NODE_ENV=production
FRONTEND_URL=http://ваш-домен.com
```

**Важно:**
- Замените `MONGODB_URI` на вашу строку подключения к MongoDB
- Замените `JWT_SECRET` на случайную строку (минимум 32 символа)
- Замените `FRONTEND_URL` на ваш домен

### 2. Установите зависимости на сервере:

```bash
cd backend
npm install --production
```

### 3. Создайте необходимые папки:

```bash
mkdir -p backend/uploads
mkdir -p photo
```

## 🚀 Шаг 3: Настройка панели управления хостинга

### Для Pterodactyl панели:

1. **MAIN FILE:** `backend/dist/server.js`

2. **Команда запуска (опционально):**
```bash
if [[ -f package.json ]]; then /usr/local/bin/npm install; fi
/usr/local/bin/node backend/dist/server.js
```

3. **Переменные окружения в панели:**
   - `NODE_ENV=production`
   - `PORT` (обычно устанавливается автоматически)
   - `MONGODB_URI` - ваша строка подключения к MongoDB
   - `JWT_SECRET` - ваш секретный ключ
   - `FRONTEND_URL` - URL вашего фронтенда

## 🔍 Шаг 4: Проверка работы

1. **Запустите сервер** через панель управления

2. **Проверьте API:**
   - Откройте: `http://ваш-домен.com/api/health`
   - Должен вернуться: `{"status":"OK","message":"Server is running"}`

3. **Проверьте фронтенд:**
   - Откройте: `http://ваш-домен.com`
   - Должен загрузиться интерфейс

## 👤 Шаг 5: Создание первого администратора

1. **Зарегистрируйтесь** через интерфейс (первый пользователь)

2. **Подключитесь к MongoDB** и выполните:
```javascript
db.users.updateOne(
  {username: "ваш_логин"}, 
  {$set: {role: "admin"}}
)
```

Или используйте MongoDB Compass или другой клиент.

## 📋 Итоговая структура на сервере:

```
/home/container/
├── backend/
│   ├── dist/
│   │   └── server.js
│   ├── uploads/
│   ├── node_modules/
│   ├── package.json
│   ├── package-lock.json
│   └── .env
├── frontend/
│   └── dist/
│       ├── index.html
│       └── assets/
└── photo/
```

## ❓ Решение проблем

### Сервер не запускается:
- Проверьте логи в панели управления
- Убедитесь, что `backend/dist/server.js` существует
- Проверьте, что все зависимости установлены
- Проверьте переменные окружения

### MongoDB не подключается:
- Проверьте `MONGODB_URI` в `.env`
- Для MongoDB Atlas: убедитесь, что IP адрес добавлен в whitelist
- Проверьте логи подключения

### Фронтенд не загружается:
- Убедитесь, что `frontend/dist/` загружен
- Проверьте, что `index.html` существует
- Проверьте консоль браузера на ошибки

## 🎉 Готово!

После выполнения всех шагов ваш проект должен работать на хостинге!

